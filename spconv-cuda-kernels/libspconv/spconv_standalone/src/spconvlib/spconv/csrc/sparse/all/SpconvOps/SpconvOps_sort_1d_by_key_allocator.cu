...
tv::Tensor SpconvOps::sort_1d_by_key_allocator(tv::Tensor data, std::function<std::uintptr_t(std::size_t)> alloc_func, tv::Tensor indices, std::uintptr_t stream, int mask_count, bool do_sort)   {
  ...
  if (data.dtype() == tv::DType(1)){
    ...
    TV_THROW_RT_ERR("unknown dtype data.dtype(), ", tv::dtype_str(data.dtype()), " available: [int32_t, int64_t, uint32_t, uint64_t]")
  }
