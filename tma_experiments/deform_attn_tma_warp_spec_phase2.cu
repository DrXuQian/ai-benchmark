...
// Phase 2: Shared metadata structure for producer-consumer communication
// Producer warps compute and write this, consumer warps read it
template <typename scalar_t>
struct SharedPointMeta {
    half2 weighthalf2;      // Attention weight replicated for half2 ops
    scalar_t h_im, w_im;    // Image coordinates (for computing bilinear weights)
    int32_t hLow, wLow;     // Integer coordinates (for array indexing)
    int within_range;       // Boundary check result
};
    ...
    cuda::barrier<cuda::thread_scope_block> warp_bars[STAGES][NUMBERS_OF_WARPS])
{
    if (is_loader_thread and within_range) {
        ...
        asm volatile(
            ...
            "}"
            :
            : "r"(static_cast<unsigned>(__cvta_generic_to_shared(&smem_tile[stage_idx][warp_id][query_id_in_warp][0][0]))),
...
__global__ void ms_deformable_im2col_gpu_kernel_template(
    ...
    // Grid-persistent loop: each block processes multiple work items
    ...
    // Warp specialization: warps 0-1 are producers (TMA load), warps 2-7 are consumers (compute)
    ...
    const bool is_consumer_warp = (warp_id >= NUM_PRODUCER_WARPS);
    ...
    // Phase 2: Shared memory for metadata exchange between producer and consumer warps
    // Since all warps in a block work on the SAME query, metadata is indexed by [point_id] only
    // Producer warps compute and write once, consumer warps read
    __shared__ SharedPointMeta<scalar_t> shared_meta[NUM_POINT];

    // Block-level barrier for producer-consumer synchronization
    ...
    // Initialize producer-consumer barrier (all threads participate)
    if (tid == 0) {
        init(&prod_cons_barrier, blockDim.x);  // All 256 threads
    }
    __syncthreads();
    ...
    for (int l_col = 0; l_col < NUM_LEVELS; ++l_col) {
      ...
      // PRODUCER WARPS: Compute metadata, write to shared memory, and issue TMA loads
      if (is_producer_warp) {
          #pragma unroll
          for (int p = 0; p < STAGES - 1; ++p) {
              ...
              // Phase 2: ONE thread per warp writes metadata to shared memory for all warps
              // Use warp_id 0, lane 0 to write (avoids race conditions)
              if (warp_id == 0 && lane_id == 0) {
                  shared_meta[p].weighthalf2 = cur_point_meta.weighthalf2;
                  shared_meta[p].h_im = cur_point_meta.h_im;
                  shared_meta[p].w_im = cur_point_meta.w_im;
                  shared_meta[p].hLow = cur_point_meta.hLow;
                  shared_meta[p].wLow = cur_point_meta.wLow;
                  shared_meta[p].within_range = cur_point_meta.within_range;
              }
      ...
      // CONSUMER WARPS: No redundant computation - will read from shared memory after barrier
      // (No action needed in initialization for consumers)

      // Synchronize: producers finish loading, consumers can start computing
      ...
      for (int p_col = 0; p_col < NUM_POINT; ++p_col) {
        ...
        // PRODUCER WARPS: Compute next point metadata, write to SMEM, and issue TMA load
        if (is_producer_warp) {
            if (p_col + STAGES - 1 < NUM_POINT) {
                ...
                // Phase 2: Write metadata to shared memory for consumers
                if (warp_id == 0 && lane_id == 0) {
                    shared_meta[next_p].weighthalf2 = next_point_meta.weighthalf2;
                    shared_meta[next_p].h_im = next_point_meta.h_im;
                    shared_meta[next_p].w_im = next_point_meta.w_im;
                    shared_meta[next_p].hLow = next_point_meta.hLow;
                    shared_meta[next_p].wLow = next_point_meta.wLow;
                    shared_meta[next_p].within_range = next_point_meta.within_range;
                }
        ...
        // CONSUMER WARPS: No metadata computation - will read from shared memory
        // (No action needed for next point)

        // All warps wait for TMA to complete
        wait_tma_load<STAGES, number_of_warps>(cur_stage_id, warp_id, warp_bars);

        // Phase 2: ALL warps (producer and consumer) read metadata from shared memory
        // This eliminates redundant computation in consumer warps
        const auto& smeta = shared_meta[p_col];
        const int within_range = smeta.within_range;

        if (within_range) {
            int hLow = smeta.hLow;
            int wLow = smeta.wLow;
            const scalar_t h_im = smeta.h_im;
            const scalar_t w_im = smeta.w_im;
            half2 weighthalf2 = smeta.weighthalf2;
