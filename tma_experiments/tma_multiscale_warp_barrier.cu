...
// Multi-Scale TMA Loading with Per-Warp Barriers
// Supports 4 different spatial scales (92x160, 46x80, 23x40, 12x20)
// Each scale has its own TMA descriptor
// Optimal performance with warp-level synchronization
...
// Host-side level configurations (will be passed to device)
__constant__ LevelConfig d_level_configs[NUM_LEVELS];

// Multi-scale TMA loading kernel
// Each query processes all 4 levels
// 8 warps handle 8 points, each point loads from all 4 levels
__global__ void tma_multiscale_kernel(
...
#pragma nv_diag_suppress static_var_with_dynamic_init
    ...
    const int p_col = warp_id;  // Each warp handles one point

    if (p_col < num_points) {
        // Process all 4 levels for this point
        for (int l_col = 0; l_col < NUM_LEVELS; l_col++) {
            if (lane_id == 0) {
                // Get sampling location for this level
            ...
            // output layout: [batch][query][levels][points][128]
...
int main() {
    printf("=== Multi-Scale TMA with Per-Warp Barriers ===\n\n");
    ...
    for (int l = 0; l < NUM_LEVELS; l++) {
        ...
        printf("  ✓ Level %d: [%d×%d], TMA descriptor created\n",
    ...
    printf("Warming up...\n");
    for (int i = 0; i < 3; i++) {
        tma_multiscale_kernel<<<num_blocks, 256>>>(
    ...
    printf("\n=== Performance Results ===\n");
    printf("Timing:\n");
    printf("  Average: %.4f ms (%.2f μs)\n", avg_time, avg_time * 1000);
    printf("  Min:     %.4f ms\n", min_time);
    printf("  Max:     %.4f ms\n\n", max_time);

    // Calculate total data transferred
    // Each query loads: 4 levels × 8 points × 256 bytes = 8192 bytes
    ...
    printf("  Total TMA operations: %d (4 levels × 8 points × 1000 queries)\n",
           NUM_LEVELS * num_points * num_blocks);
    printf("  TMA ops/sec: %.2f M ops/s\n\n",
           (NUM_LEVELS * num_points * num_blocks) / (avg_time / 1000.0) / 1e6);

    // Verify correctness for first level
    printf("Verifying correctness (Level 0)...\n");
    const int num_verify = 100;
    ...
    for (int bid = 0; bid < num_verify; bid++) {
        const int b_col = bid / num_query;
        const int q_col = bid % num_query;
    ...
    printf("\n✅ Multi-scale TMA test completed!\n");
    return 0;
}
