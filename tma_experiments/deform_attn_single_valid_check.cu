  ...
  for (int l_col = 0; l_col < NUM_LEVEL; ++l_col) {
    ...
    if constexpr (UNROLL_FACTOR == 4) {
      ...
      for (int batch = 0; batch < NUM_POINT; batch += 4) {
        // Allocate separate registers for each corner's data
        __half vdata_corner0[4][NUM_OUTPUT];
        __half vdata_corner1[4][NUM_OUTPUT];
        __half vdata_corner2[4][NUM_OUTPUT];
        __half vdata_corner3[4][NUM_OUTPUT];

        half2 weighthalf2[4];
        __half wdata[4][4];
        bool valid[4];

        // Compute metadata and load all data for valid points
        #pragma unroll
        for (int i = 0; i < 4; ++i) {
          const int p = batch + i;
          ...
          if (valid[i]) {
            ...
            // Load data for all 4 corners at once
            int32_t ptr0 = (hLowPtrOffset + wLowPtrOffset + c_col) / NUM_OUTPUT;
            int32_t ptr1 = (hLowPtrOffset + wHighPtrOffset + c_col) / NUM_OUTPUT;
            int32_t ptr2 = (hHighPtrOffset + wLowPtrOffset + c_col) / NUM_OUTPUT;
            int32_t ptr3 = (hHighPtrOffset + wHighPtrOffset + c_col) / NUM_OUTPUT;

            LDST128BITS(vdata_corner0[i][0]) = LDST128BITS(const_cast<__half*>(data_value_ptr)[ptr0]);
            LDST128BITS(vdata_corner1[i][0]) = LDST128BITS(const_cast<__half*>(data_value_ptr)[ptr1]);
            LDST128BITS(vdata_corner2[i][0]) = LDST128BITS(const_cast<__half*>(data_value_ptr)[ptr2]);
            LDST128BITS(vdata_corner3[i][0]) = LDST128BITS(const_cast<__half*>(data_value_ptr)[ptr3]);
          }
        }

        // Now process all corners without any valid checks
        // Corner 0
        #pragma unroll
        for (int i = 0; i < 4; ++i) {
          if (valid[i]) {
            half2 wdataexp = __hmul2(half2(wdata[i][0], wdata[i][0]), weighthalf2[i]);
            #pragma unroll
            for (int j = 0; j < NUM_OUTPUT; j += 2) {
              HALF2(col[j]) = __hfma2(wdataexp, HALF2(vdata_corner0[i][j]), HALF2(col[j]));
            }
          }
        }
        ...
        for (int i = 0; i < 4; ++i) {
          if (valid[i]) {
            half2 wdataexp = __hmul2(half2(wdata[i][1], wdata[i][1]), weighthalf2[i]);
            #pragma unroll
            for (int j = 0; j < NUM_OUTPUT; j += 2) {
              HALF2(col[j]) = __hfma2(wdataexp, HALF2(vdata_corner1[i][j]), HALF2(col[j]));
            }
          }
        }
        ...
        for (int i = 0; i < 4; ++i) {
          if (valid[i]) {
            half2 wdataexp = __hmul2(half2(wdata[i][2], wdata[i][2]), weighthalf2[i]);
            #pragma unroll
            for (int j = 0; j < NUM_OUTPUT; j += 2) {
              HALF2(col[j]) = __hfma2(wdataexp, HALF2(vdata_corner2[i][j]), HALF2(col[j]));
            }
          }
        }
        ...
        for (int i = 0; i < 4; ++i) {
          if (valid[i]) {
            half2 wdataexp = __hmul2(half2(wdata[i][3], wdata[i][3]), weighthalf2[i]);
            #pragma unroll
            for (int j = 0; j < NUM_OUTPUT; j += 2) {
              HALF2(col[j]) = __hfma2(wdataexp, HALF2(vdata_corner3[i][j]), HALF2(col[j]));
            }
          }
        }
      }
      ...
      for (int batch = 0; batch < NUM_POINT; batch += 2) {
        __half vdata_corner0[2][NUM_OUTPUT];
        __half vdata_corner1[2][NUM_OUTPUT];
        __half vdata_corner2[2][NUM_OUTPUT];
        __half vdata_corner3[2][NUM_OUTPUT];
        ...
        // Process corners without additional valid checks
        #pragma unroll
        for (int i = 0; i < 2; ++i) {
          if (valid[i]) {
            // Corner 0
            ...
            // Corner 1
            wdataexp = __hmul2(half2(wdata[i][1], wdata[i][1]), weighthalf2[i]);
            #pragma unroll
            for (int j = 0; j < NUM_OUTPUT; j += 2) {
              HALF2(col[j]) = __hfma2(wdataexp, HALF2(vdata_corner1[i][j]), HALF2(col[j]));
            }
            ...
            // Corner 2
            wdataexp = __hmul2(half2(wdata[i][2], wdata[i][2]), weighthalf2[i]);
            #pragma unroll
            for (int j = 0; j < NUM_OUTPUT; j += 2) {
              HALF2(col[j]) = __hfma2(wdataexp, HALF2(vdata_corner2[i][j]), HALF2(col[j]));
            }
            ...
            // Corner 3
            wdataexp = __hmul2(half2(wdata[i][3], wdata[i][3]), weighthalf2[i]);
            #pragma unroll
            for (int j = 0; j < NUM_OUTPUT; j += 2) {
              HALF2(col[j]) = __hfma2(wdataexp, HALF2(vdata_corner3[i][j]), HALF2(col[j]));
            }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Single Valid Check - Unroll Factor " << unroll_factor << std::endl;
